...
/**
    ...
    Complexity:
        the row will be O(log(m)),m is the total row of matrix,we can get the location(specified row) which target may exist.
...
class Solution {
       ...
       public:
            ...
            bool searchLine(vector<vector<int> >&nums,int target,int bline,int eline){
                 ...
                 else if(target < nums[midline][0])//lesser than first one
                    return  searchLine(nums,target,bline,midline-1);
                 else
                    return binary_search(nums[midline],target);
