       ...
       public:
            ...
            bool searchMatrix(vector<vector<int>>& matrix, int target) {
                    ...
                    while(leftrow>=0 && leftcol < n){
                        if(target == matrix[leftrow][leftcol])
                            return 1;
                        else if (target > matrix[leftrow][leftcol])
                            leftcol++;
                        else
                            leftrow--;
                    }
...
//                    cout<<"begin searching..."<<endl;
                    ...
                    return searchTarget(matrix,m-1,0,target);

            }

            bool searchTarget(vector<vector<int>>& nums,int leftrow,int leftcol,int target){
                if(leftrow < 0 || leftcol > lastcol)
                    return 0;

                if(leftrow == 0 && leftcol == lastcol)
                    return target == nums[0][lastcol];
                // if(leftcol == lastcol)
                //     return 0;
//                cout<<leftrow<<"->"<<leftcol<<"---->>> "<<nums[leftrow][leftcol]<<endl;

                if(nums[leftrow][leftcol] == target)
                    return 1;
                else
                    if(nums[leftrow][leftcol] < target)
                        return searchTarget(nums,leftrow,leftcol+1,target);
                else
                    return searchTarget(nums,leftrow-1,leftcol,target);
